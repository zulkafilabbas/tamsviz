...
BUTTON_WIDTH = 22
BUTTON_HEIGHT = 3  # taller for multiline
WRAP_LEN = 180  # wrap long text
    ...
    def create_widgets(self):
        ...
        for category in self.config['categories']:
            ...
            for col, action in enumerate(category['actions']):
                btn = tk.Button(
                    button_frame, text=action,
                    font=BUTTON_FONT, width=BUTTON_WIDTH, height=BUTTON_HEIGHT,
                    wraplength=WRAP_LEN,
                    relief="groove", bd=2, bg="lightgrey",
                    command=lambda a=action, c=category['name']: self.on_button_click(a, c)
                )
...
def main():
    ...
    with open(os.path.join(os.path.dirname(__file__), 'label_selector_config3.json'), 'r') as f:
        config = json.load(f)
